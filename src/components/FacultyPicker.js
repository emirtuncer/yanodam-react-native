...
const FacultyPicker = ({style}) => {
  const [user, setUser] = useContext(UserContext);
  const [faculty, setFaculty] = useState();
  const [itemIndexState, setItemIndex] = useState();
  const [itemValueState, setItemValue] = useState();
  const [itemLabelState, setItemLabel] = useState();

  const pickersListArray = faculties.map(obj => (
